...
def _build_command_usage(help_dict, command_key):
    ...
    for i, page in enumerate(cmd_pages):
        ...
        buttons.data_button("סגירה", "help close", "footer", style=ButtonStyle.DANGER)
...
def bt_selection_controls(id_):
    """מחזיר יחד את הודעת בחירת הקבצים ואת הכפתורים התואמים לה.

    הקוד מוצג במפורש בהודעת Telegram ולא נשען רק על callback. כך המשתמש
    מקבל אותו גם אם חלונית ההתראה של Telegram אינה נפתחת או שכפתור הקוד הוחמץ.
    הערך הטכני עטוף בבידוד LTR כדי שארבע הספרות לא יתהפכו בתוך טקסט עברי.
    """

    text = (
        "\u200f⏸️ <b>ההורדה הושהתה לבחירת קבצים</b>\n\n"
        "פתחו את מסך בחירת הקבצים, סמנו מה להוריד ולחצו "
        "<b>סיום הבחירה</b>."
    )
    if Config.WEB_PINCODE:
        pin = derive_pin(id_, _resolve_bot_id())
        text += (
            "\n\n🔐 <b>קוד הגישה:</b> "
            f"<code>\u2066{pin}\u2069</code>\n"
            "הזינו את הקוד באתר בדיוק כפי שהוא מופיע כאן."
        )
    return text, bt_selection_buttons(id_)
