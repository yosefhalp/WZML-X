...
from .ext_utils.telegram_delivery import (
    ...
    resolve_private_transmission_mode,
)
...
class TaskConfig:
    def __init__(self):
        ...
        # מזהה פנימי בלבד שמאפשר להוסיף תוצאה לתיקיית GoFile שמורה. הטוקן
        # עצמו אינו עובר בפקודה, בהודעה או ב-callback.
        self.gofile_target_id = getattr(self.message, "_gofile_target_id", "")
        self.progress = True
    ...
    async def before_start(self):
        ...
        if not self.is_leech:
            ...
            if not gc_used:
                ...
                if self.is_uphoster and not self.up_dest:
                    ...
                    for service in services:
                        if service == "gofile":
                            # GoFile מחזיר guestToken בהעלאה הראשונה. אין לדרוש
                            # טוקן קיים ואין להשתמש בבריכת טוקנים חיצונית.
                            continue
            ...
            if self.hybrid_leech:
                self.transmission_mode = "both"
            if self.user_trans and TgClient.user is None:
                raise ValueError("ה־Userbot שנבחר כמקור העלאה אינו מחובר")

            # יעד שנבחר במפורש באשף גובר על יעד dump גלובלי. אחרת בחירת
            # "פרטי" עלולה להישלח לערוץ ישן או ל-Saved Messages.
            self.up_dest = "" if self.cmd_up_dest else Config.LEECH_DUMP_CHAT
            if self.cmd_up_dest and self.cmd_up_dest != self.user_id:
                route_client = (
                    TgClient.user if self.transmission_mode == "user" else self.client
                )
                if route_client is None:
                    raise ValueError("מקור ההעלאה שנבחר אינו מחובר כרגע")
                try:
                    route_chat = await route_client.get_chat(self.cmd_up_dest)
                except Exception as error:
                    raise ValueError(
                        "הערוץ לא נמצא אצל מקור ההעלאה שנבחר"
                    ) from error
                if route_chat.type not in [ChatType.CHANNEL, ChatType.SUPERGROUP]:
                    raise ValueError("יעד Telegram שנבחר אינו ערוץ")
                try:
                    route_member = await route_chat.get_member(route_client.me.id)
                except Exception as error:
                    raise ValueError("לא ניתן לבדוק הרשאות פרסום בערוץ") from error
                privileges = getattr(route_member, "privileges", None)
                can_post = bool(
                    getattr(privileges, "can_post_messages", False)
                    or getattr(privileges, "can_manage_chat", False)
                    or str(getattr(route_member, "status", ""))
                    .casefold()
                    .endswith("owner")
                )
                if not can_post:
                    raise ValueError(
                        "למקור ההעלאה שנבחר אין הרשאת פרסום בערוץ"
                    )
            if is_saved_messages_destination(self.up_dest, self.user_id):
                # מזהה הבעלים הוא "הודעות שמורות" מנקודת מבט ה־Userbot.
                # במסלול פרטי הקובץ צריך להגיע לבוט עצמו, לכן היעד הישן מנוטרל.
                LOGGER.info("יעד Saved Messages הוחלף במסירה ישירה אל הבוט")
                self.up_dest = ""
            if self.up_dest:
                ...
                self.transmission_mode = resolve_private_transmission_mode(
                    self.transmission_mode,
                    user_available=TgClient.user is not None,
                )
            if self.split_size:
            ...
            LOGGER.info(
                "תוכנית מסירת Telegram: mode=%s premium=%s split=%s max=%s",
                self.transmission_mode,
