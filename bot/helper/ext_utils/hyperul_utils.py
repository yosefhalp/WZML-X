...
from ..ext_utils.telegram_delivery import (
    ...
    eligible_hyper_client_keys,
    resolve_upload_chat_id,
)
...
class HypertgUpload(HypertgTransfer):
    ...
    async def upload(
        ...
        self._cancel.clear()
        upload_name = ospath.basename(file_path)
        ...
        if not is_image and thumb is None:
            file_name = ospath.splitext(upload_name)[0]
        ...
        eligible_keys = eligible_hyper_client_keys(
            self.clients, user_session=user_session
        )
        # מאגר Hyper עשוי להכיל רק Userbot. במקרה כזה משימת Bot חייבת
        # ליפול ללקוח הבוט הישיר ולא לבחור בטעות את חשבון המשתמש.
        if use_hyper and not eligible_keys:
            use_hyper = False
        upload_chat_id = resolve_upload_chat_id(
            user_session=user_session,
            private_chat=reply_target.chat.type == ChatType.PRIVATE,
            ...
            fallback_chat_id=reply_target.chat.id,
        )
        LOGGER.info(
            "מסלול העלאת Telegram: client=%s target=%s private=%s size=%s",
            "user" if user_session else "bot",
            "bot" if upload_chat_id != reply_target.chat.id else "origin",
            reply_target.chat.type == ChatType.PRIVATE,
            up_size,
        )
        try:
            if use_hyper:
                ...
                sent, actual_user_session = await self._hyper_send(
                    file_path,
                    key,
                    thumb,
                    cap_mono,
                ...
                actual_user_session = bool(user_session)

            LOGGER.info(f"HypertgUL uploaded {upload_name}")
            return UploadDeliveryReceipt(
                message=sent,
                client_kind="user" if actual_user_session else "bot",
                requested_chat_id=upload_chat_id,
            )

        except StopTransmission:
            LOGGER.warning(f"HypertgUL cancelled {upload_name}")
            raise
        except Exception as e:
            LOGGER.error(f"HypertgUL fail {upload_name}: {type(e).__name__}: {e}")
            raise
        finally:
    ...
    async def _send_with_retry(self, send_func, upload_name, **kwargs):
        while True:
            try:
                return await send_func(**kwargs)
            except (FloodWait, FloodPremiumWait) as f:
                LOGGER.warning(f"HypertgUL flood {f.value}s on {upload_name}")
                await sleep(f.value + 1)

    async def _try_send(self, key, client, kwargs, upload_name):
        try:
            if key == "videos":
                return await self._send_with_retry(
                    client.send_video, upload_name, **kwargs
                )
            elif key == "audios":
                return await self._send_with_retry(
                    client.send_audio, upload_name, **kwargs
                )
            elif key == "photos":
                return await self._send_with_retry(
                    client.send_photo, upload_name, **kwargs
                )
            else:
                return await self._send_with_retry(
                    client.send_document, upload_name, **kwargs
                )
    ...
    async def _hyper_send(
        ...
        candidates = {
            key: self.work_loads[key]
            for key in eligible_keys or []
            if key in self.clients and key in self.work_loads
        }
        if not candidates:
            raise RuntimeError("לא נמצא לקוח Hyper מהסוג שנבחר למשימה")
        ...
        try:
            ...
            sent = await self._try_send(
                key, client, kwargs, ospath.basename(file_path)
            )
            return sent, idx < 0
        finally:
            self.work_loads[idx] -= 1
