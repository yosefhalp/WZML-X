...
from ..helper.telegram_helper.rich_message import send_rich_message
...
async def imdb_callback(_, query):
    ...
    if user_id != int(data[1]):
        ...
        if production_companies:
            ...
            prod_html = f"""
<details>
<summary>חברות הפקה</summary>
<ul>{prod_items}</ul>
</details>"""

        ratings_rows = ""
        if rating and rating != "N/A":
            ratings_rows += f"<tr><td>IMDb</td><td>{rating} ({votes} הצבעות)</td></tr>"
        if metascore:
...
<caption>דירוגים</caption>
<tr><th>מקור</th><th>ציון</th></tr>
{ratings_rows}
</table>"""

        info_rows = ""
        if genres_plain and genres_plain != "N/A":
            info_rows += f"<tr><td><b>סוגות</b></td><td>{genres_plain}</td></tr>"
        rel = " | ".join(filter(None, [release_date, release_country]))
        if rel:
            info_rows += f"<tr><td><b>יציאה</b></td><td>{rel}</td></tr>"
        if countries_plain and countries_plain != "N/A":
            info_rows += f"<tr><td><b>מדינות</b></td><td>{countries_plain}</td></tr>"
        if languages_plain and languages_plain != "N/A":
            info_rows += f"<tr><td><b>שפות</b></td><td>{languages_plain}</td></tr>"
        info_table = ""
        if info_rows:
            info_table = f"""
<table striped>
<caption>מידע</caption>
{info_rows}
</table>"""
        ...
        if director and director != "N/A":
            credits_items += f"<li><b>בימוי</b> — {director}</li>"
        if creators and creators != "N/A":
            credits_items += f"<li><b>יוצרים</b> — {creators}</li>"
        if writer and writer != "N/A":
            credits_items += f"<li><b>כתיבה</b> — {writer}</li>"
        if cast and cast != "N/A":
            credits_items += f"<li><b>שחקנים ראשיים</b> — {cast}</li>"
        credits_html = ""
        if credits_items:
            credits_html = f"""
<h3>יוצרים ומשתתפים</h3>
<ul>{credits_items}</ul>"""

        box_office_rows = ""
        if budget:
            box_office_rows += f"<tr><td><b>תקציב</b></td><td><code>{budget}</code></td></tr>"
        if box_opening:
            box_office_rows += f"<tr><td><b>סוף שבוע ראשון</b></td><td><code>{box_opening}</code></td></tr>"
        if box_domestic:
            box_office_rows += f"<tr><td><b>הכנסות מקומיות</b></td><td><code>{box_domestic}</code></td></tr>"
        if box_office and box_office != "N/A":
            box_office_rows += f"<tr><td><b>הכנסות עולמיות</b></td><td><code>{box_office}</code></td></tr>"
        box_office_table = ""
        if box_office_rows:
            box_office_table = f"""
<table bordered striped>
...
<details>
<summary><b>עלילה — לחצו לפתיחה, כולל ספוילרים</b></summary>
...
<a href="{url}">פתיחה ב־IMDb</a>"""

            fallback = (
                f"<b>{title} ({year_text})</b>\n\n"
                f"{tagline}\n\n{plot_full}\n\n"
                f'<a href="{url}">פתיחה ב־IMDb</a>'
            )
            await send_rich_message(reply_to, rich_html, fallback, buttons)
        await delete_message(message)
    else:
        await query.answer()
