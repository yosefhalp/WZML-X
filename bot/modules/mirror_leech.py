...
class Mirror(TaskListener):
    ...
    async def new_event(self):
        ...
        self.link = normalize_external_link(args["link"])
        ...
        if not self.link and (reply_to := self.message.reply_to_message):
            if reply_to.text:
                self.link = normalize_external_link(reply_to.text.split("\n", 1)[0])
        ...
        if reply_to:
            ...
            if file_ is None:
                if reply_text := reply_to.text:
                    self.link = normalize_external_link(reply_text.split("\n", 1)[0])
                else:
                    reply_to = None
        ...
        self.link = normalize_external_link(self.link)
...
async def qb_uphoster(client, message):
    """מוריד דרך qBittorrent ומעלה את התוצאה ל־GoFile."""

    bot_loop.create_task(
        Mirror(client, message, is_qbit=True, is_uphoster=True).new_event()
    )


async def jd_mirror(client, message):
    if Config.DISABLE_JD:
        await message.reply("מנוע JDownloader מושבת כרגע בהגדרות הבוט.")
        return
    bot_loop.create_task(Mirror(client, message, is_jd=True).new_event())


async def jd_uphoster(client, message):
    """מוריד דרך JDownloader ומעלה ל־GoFile רק לאחר שחיבור MyJD הוגדר."""

    if Config.DISABLE_JD or not Config.JD_EMAIL or not Config.JD_PASS:
        await message.reply(
            "JDownloader אינו מוגדר. יש לשמור כתובת דוא״ל וסיסמה של MyJDownloader ולהפעיל את המנוע."
        )
        return
    bot_loop.create_task(
        Mirror(client, message, is_jd=True, is_uphoster=True).new_event()
    )


async def _start_nzb_task(client, message, *, is_uphoster=False):
    """מכין קישור NZB רגיל או מזהה Hydra ומפעיל את יעד ההעלאה שנבחר."""

    if Config.DISABLE_NZB:
        await message.reply("מנוע SABnzbd / Usenet מושבת כרגע בהגדרות הבוט.")
        return
    ...
    if len(text_parts) > 1 and not text_parts[1].startswith(("http", "ftp", "/")):
        ...
        if "-e" not in message.text:
            message.text += " -e"
    mirror_task = Mirror(client, message, is_nzb=True, is_uphoster=is_uphoster)
    if nzb_id:
...
async def nzb_mirror(client, message):
    await _start_nzb_task(client, message)


async def nzb_uphoster(client, message):
    """מוריד קישור NZB דרך SABnzbd ומעלה את התוצאה ל־GoFile."""

    await _start_nzb_task(client, message, is_uphoster=True)
