...
def _update_sabnzbd_ini(api_key):
    ...
    pat_user = _re(r"^username\s*=.*$", MULTILINE)
    pat_host = _re(r"^host\s*=.*$", MULTILINE)
    try:
        with open("configs/sabnzbd/SABnzbd.ini", "r+") as f:
            content = f.read()
            new = content
            new = pat_key.sub(f"api_key = {api_key}", new)
            # ממשק SABnzbd מאזין רק בתוך הקונטיינר. ההזדהות החיצונית נשארת
            # ב־proxy של WZML-X, ולכן אין מסך כניסה פנימי נוסף או סיסמה כפולה.
            new = pat_pwd.sub("password = ", new)
            new = pat_user.sub("username = ", new)
            new = pat_host.sub("host = 127.0.0.1", new)
            if new == content:
                return
            f.seek(0)
            f.truncate()
