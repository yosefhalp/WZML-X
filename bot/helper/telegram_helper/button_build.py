...
from ..languages.he_ui import translate_button
...
class ButtonMaker:
    ...
    def url_button(self, key, link, position=None, style=None):
        self.buttons[position if position in self.buttons else "default"].append(
            InlineKeyboardButton(
                text=translate_button(key), url=link, style=_btn_style(style)
            )
        )
    ...
    def data_button(self, key, data, position=None, style=None):
        self.buttons[position if position in self.buttons else "default"].append(
            InlineKeyboardButton(
                text=translate_button(key), callback_data=data, style=_btn_style(style)
            )
        )
