...
from ...ext_utils.telegram_delivery import (
    build_upload_reply_context,
    delivery_receipt_matches_route,
    delivered_upload_count,
    is_saved_messages_destination,
    normalize_delivery_chat_id,
    upload_delivery_complete,
)
...
class TelegramUploader:
    ...
    async def _msg_to_reply(self):
        ...
        # כל החלקים חייבים לחשב יעד מאותה הודעת מקור. ``_sent_msg`` משתנה
        # בהמשך לצורכי רצף ומדיה, ולכן אסור להשתמש בו כבסיס לניתוב מקבילי.
        self._reply_context = build_upload_reply_context(self._sent_msg)
        return True
        ...
        for entry in self._upload_seq:
            ...
            for dest_attr in ("cmd_up_dest", "leech_dest"):
                ...
                if not dest or dest == self._listener.up_dest:
                    continue
                if is_saved_messages_destination(dest, self._listener.user_id):
                    LOGGER.info("דילוג על העתקה כפולה אל Saved Messages")
                    continue
    ...
    async def upload(self):
        ...
        if upload_tasks:
            ...
            for r in results:
                if isinstance(r, Exception):
                    ...
                    successful_messages.append(r)
            # ``gather`` שומר את סדר הקבצים גם אם הם הסתיימו בסדר אחר.
            # רק לאחר שכל ההעלאות הסתיימו מעדכנים את ההודעה האחרונה.
            if successful_messages:
                self._sent_msg = successful_messages[-1]
            await sleep(1)
        for key, value in list(self._media_dict.items()):
            for subkey, msgs in list(value.items()):
                if len(msgs) > 1:
        ...
        if self._upload_seq:
            ...
            # לצ'אט פרטי אין קישור ציבורי. סינון ערכים ריקים מונע יצירת
            # תגית HTML שמפנה בטעות לכתובת המילולית "None".
            self._msgs_dict = {
                e["link"]: e["file_"]
                for e in self._upload_seq
                if e is not None and e["link"]
            }
        if self._listener.is_cancelled:
            return
        if self._total_files == 0:
            await self._listener.on_upload_error(
                "לא נמצאו קבצים להעלאה. אם הוגדרו סיומות מוחרגות, יש לבדוק שלא כל הקבצים סוננו."
            )
            return
        if not upload_delivery_complete(self._upload_seq, self._total_files):
            delivered = delivered_upload_count(self._upload_seq)
            await self._listener.on_upload_error(
                "ההעלאה אינה מלאה ולכן המשימה לא סומנה כהצלחה. "
                f"אומתו {delivered} מתוך {self._total_files} חלקים ב־Telegram."
            )
            return
        if self._total_files <= self._corrupted:
            await self._listener.on_upload_error(
                f"הקבצים פגומים או שלא ניתן היה להעלות אותם. {self._error or 'יש לבדוק את הלוגים.'}"
            )
            return
    ...
    async def _upload_file(
        ...
        if self._reply_context is None:
            LOGGER.error("Cannot upload: immutable reply context is missing")
            await self._listener.on_upload_error(
                "Upload failed: Message not initialized"
            )
            return

        if self._reply_context.target.chat is None:
        ...
        try:
            ...
            receipt = await self._hu.upload(
                file_path=o_path,
                cap_mono=cap_mono,
                reply_target=self._reply_context.target,
                reply_to_message_id=self._reply_context.message_id,
                force_document=force_document,
                user_session=user_session,
                user_thumb=self._thumb,
            )
            sent_msg = receipt.message

            if not delivery_receipt_matches_route(
                receipt,
                expected_client_kind="user" if user_session else "bot",
                private_chat=self._is_private,
                owner_id=self._listener.user_id,
            ):
                raise RuntimeError(
                    "קבלת המסירה אינה תואמת למקור וליעד שנבחרו; "
                    "החלק לא ייספר כהעלאה מוצלחת"
                )

            if self._listener.is_cancelled:
                return

            bot_view_chat_id = normalize_delivery_chat_id(
                sent_chat_id=sent_msg.chat.id,
                owner_id=self._listener.user_id,
                user_session=user_session,
                private_chat=self._is_private,
                explicit_destination=self._listener.cmd_up_dest,
                dump_destination=Config.LEECH_DUMP_CHAT,
                bot_peer=f"@{TgClient.BNAME}" if TgClient.BNAME else TgClient.ID,
            )
            self._upload_seq[seq_idx] = {
                "chat_id": bot_view_chat_id,
                "msg_id": sent_msg.id,
                "link": sent_msg.link,
                "file_": file_,
            }
            self._msg_to_seq[(bot_view_chat_id, sent_msg.id)] = seq_idx
    ...
    async def cancel_task(self):
        ...
        await self._listener.on_upload_error("ההעלאה בוטלה לפי בקשת המשתמש.")
