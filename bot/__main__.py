...
from .helper.languages.he_ui import install_query_translation

Config.load()
install_query_translation()
...
async def main():
    ...
    bot_loop.create_task(TgClient.telegram_health_watchdog())
...
async def restart_sessions_confirm(_, query):
    ...
    if data[1] == "confirm":
        ...
        restart_message = await send_message(reply_to, "בודק את חיבורי Telegram...")
        await delete_message(message)
        try:
            await TgClient.reload()
            await edit_message(restart_message, "✅ כל חיבורי Telegram הפעילים תקינים.")
        except Exception as error:
            LOGGER.error("בדיקת Sessions נכשלה: %s", type(error).__name__)
            await edit_message(
                restart_message,
                "❌ לפחות חיבור Telegram אחד אינו פעיל. פרטי התקלה נשמרו בלוג.",
            )
    else:
        await delete_message(message)
...
# כתובות הגישה מכילות אסימונים נגזרים ולכן אסור לרשום אותן בלוג.
LOGGER.info("Web UI: qBittorrent and SABnzbd links are ready in the dashboard")

from .helper.ext_utils.task_control import task_request_worker

bot_loop.create_task(task_request_worker())
