...
async def restart_bot(_, message):
    ...
    buttons.data_button("כן, להפעיל מחדש", f"botrestart confirm {mode}", style=ButtonStyle.SUCCESS)
    buttons.data_button("לא, לחזור", "botrestart cancel", style=ButtonStyle.DANGER)
    button = buttons.build_menu(2)
    msg = "<i>להפעיל מחדש את הבוט?</i>"
    if mode == "soft":
        msg += "\n<i>מצב רך: הקוד ייטען מחדש והמשימות הפעילות ימשיכו לפעול.</i>"
...
async def restart_sessions(_, message):
    buttons = button_build.ButtonMaker()
    buttons.data_button("כן, להפעיל מחדש", "sessionrestart confirm", style=ButtonStyle.SUCCESS)
    buttons.data_button("לא, לחזור", "sessionrestart cancel", style=ButtonStyle.DANGER)
    button = buttons.build_menu(2)
    await send_message(
        message,
        "<i>להפעיל מחדש את חיבורי המשתמשים?</i>",
        button,
    )


def _restart_header(now, is_restart_chat=False):
    title = "ההפעלה מחדש הושלמה בהצלחה" if is_restart_chat else "הבוט הופעל מחדש"
    return (
        f"⌬ <b><i>{title}</i></b>\n"
        f"┟ <b>תאריך:</b> {now.strftime('%d/%m/%y')}\n"
        f"┠ <b>שעה:</b> {now.strftime('%H:%M:%S')}\n"
        f"┠ <b>אזור זמן:</b> {Config.TIMEZONE}\n"
        f"┠ <b>ענף:</b> {Config.UPSTREAM_BRANCH}\n"
        f"┖ <b>גרסה:</b> {get_version()}"
    )
...
async def confirm_restart(_, query):
    ...
    if data[1] == "confirm":
        ...
        if data[2] == "soft":
            restart_msg = await send_message(reply_to, "<i>טוען מחדש...</i>")
            ...
            restart_message = await send_message(reply_to, "<i>מפעיל מחדש...</i>")
