...
async def add_handlers():
    TgClient.bot.add_handler(
        CallbackQueryHandler(
            guided_callback, filters=regex("^ui ") & CustomFilters.authorized
        ),
        group=-2,
    )
    TgClient.bot.add_handler(
        MessageHandler(
            guided_input,
            filters=private & ~regex(r"^/") & CustomFilters.authorized,
        ),
        group=2,
    )
    TgClient.bot.add_handler(
        MessageHandler(
            menu_command,
            filters=command(BotCommands.MenuCommand, case_sensitive=True)
    ...
    TgClient.bot.add_handler(
        MessageHandler(
            server_command,
            filters=command(BotCommands.ServerCommand, case_sensitive=True)
            & CustomFilters.authorized,
        )
    )
    TgClient.bot.add_handler(
    ...
    if Config.SET_COMMANDS:
        # תפריט הפקודות של Telegram נשאר קצר; כל היכולות המתקדמות
        # ממשיכות לעבוד ומופיעות בתוך קטלוג הכפתורים של הבוט.
        await TgClient.bot.set_bot_commands(
            [
                BotCommand("start", "פתיחת מרכז השליטה"),
                BotCommand("menu", "חזרה לתפריט הראשי"),
                BotCommand("gofile", "העלאה מהירה ל־GoFile"),
                BotCommand("status", "המשימות הפעילות שלי"),
                BotCommand("server", "מצב השרת, השירותים והגיבוי"),
                BotCommand("usetting", "ההגדרות האישיות שלי"),
                BotCommand("help", "עזרה ויכולות מתקדמות"),
            ]
        )
