...
async def rss_update(_, message, pre_event, state):
    ...
    for title in titles:
        ...
        if not (res := rss_dict[user_id].get(title, False)):
            ...
            if not res:
                user_id = message.from_user.id
                await send_message(message, f"מקור ה־RSS בשם {title} לא נמצא.")
                continue
        istate = rss_dict[user_id][title].get("paused", False)
        if istate and state == "pause" or not istate and state == "resume":
            state_he = "מושהה" if state == "pause" else "פעיל"
            await send_message(message, f"מקור ה־RSS בשם {title} כבר {state_he}.")
            continue
        async with rss_dict_lock:
    ...
    if updated:
        ...
        await send_message(
            message,
            f"מקורות ה־RSS הבאים עודכנו: <code>{updated}</code>.",
        )
...
async def rss_get(_, message, pre_event):
    ...
    if len(args) < 2:
        await send_message(
            message,
            f"הקלט {args} אינו תקין. יש לציין שם מקור ומספר פריטים לקבלה; אפשר להיעזר במסך העזרה.",
        )
        await update_rss_menu(pre_event)
        return
    try:
...
async def rss_listener(client, query):
    ...
    if int(data[2]) != user_id and not await CustomFilters.sudo("", query):
        ...
        if len(rss_dict.get(int(data[2]), {})) == 0:
            ...
            await edit_message(
                message,
                "שלחו שם מקור ומספר פריטים, מופרדים ברווח.\nלדוגמה: <code>MyFeed 5</code>\nזמן קצוב: 60 שניות.",
                button,
            )
        ...
        if len(rss_dict.get(int(data[2]), {})) == 0:
            ...
            await edit_message(
                message,
                f"שלחו שם אחד או יותר של מקורות RSS, מופרדים ברווח, כדי לבצע {action_he}.\nזמן קצוב: 60 שניות.",
                button,
            )
